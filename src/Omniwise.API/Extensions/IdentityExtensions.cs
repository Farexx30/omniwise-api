...
// These are customized identity api endpoints from the ASP.NET Core Identity source code.
...
public static class IdentityExtensions
{
    ...
    public static IEndpointConventionBuilder MapOmniwiseIdentityApi<TUser>(this IEndpointRouteBuilder endpoints)
        where TUser : User, new()
    {
        ...
        routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>>
            ([FromBody] OmniwiseRegisterRequest registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            var userManager = sp.GetRequiredService<UserManager<TUser>>();

            if (!userManager.SupportsUserEmail)
            {
                throw new NotSupportedException($"{nameof(MapOmniwiseIdentityApi)} requires a user store with email support.");
            }
            ...
            var roleName = registration.RoleName.Trim();
            ...
            {
                return CreateValidationProblem(IdentityResult.Failed(userManager.ErrorDescriber.InvalidFirstName(firstName)));
            }

            //Check if LastName is not empty:
            if (string.IsNullOrEmpty(lastName))
            {
                return CreateValidationProblem(IdentityResult.Failed(userManager.ErrorDescriber.InvalidLastName(lastName)));
            }
            ...
            if (!roleExists)
            {
                return CreateValidationProblem(IdentityResult.Failed(userManager.ErrorDescriber.InvalidRoleName(roleName)));
            }
            ...
            //Add custom properties from the registration request:
            user.FirstName = registration.FirstName;
            user.LastName = registration.LastName;

            var userCreationResult = await userManager.CreateAsync(user, registration.Password);

            if (!userCreationResult.Succeeded)
            {
                return CreateValidationProblem(userCreationResult);
            }
            ...
            {
                //If the role assignment fails, we need to delete the user to avoid leaving an unassigned user in the database:
                await userManager.DeleteAsync(user);

                return CreateValidationProblem(roleAssignmentResult);
            }
        ...
        .Add(endpointBuilder =>
        {
            ...
            confirmEmailEndpointName = $"{nameof(MapOmniwiseIdentityApi)}-{finalPattern}";
